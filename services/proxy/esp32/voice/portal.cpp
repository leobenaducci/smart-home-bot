...
// lives in ../menu/portal.cpp and ../control/portal.cpp; a fix here is a fix
...
// device already sitting on a shelf.
...
bool settingsLoad(Settings& out) {
  ...
  out.url = prefs.getString("url", VOICE_URL);
  out.token = prefs.getString("token", VOICE_DEVICE_TOKEN);
  prefs.end();
  return out.complete();
}
...
<h1>Alfred de la casa</h1><p>Configuraci&oacute;n de red</p>
...
<label>URL del gateway de voz</label><input name=url value="%URL%" required>
<label>Token del dispositivo</label><input name=token value="%TOKEN%" required>
...
static void handleSave() {
  ...
  server.send(200, "text/html; charset=utf-8",
              "<meta charset=utf-8><p>Guardado. Alfred se reinicia…");
...
PortalInfo portalBegin() {
  ...
  char name[32];
  snprintf(name, sizeof(name), "AlfredVoice-%02X%02X", mac[4], mac[5]);
