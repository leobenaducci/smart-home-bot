...
// secrets.h is optional: with it you can flash a pre-provisioned panel, without
// it the portal is the only way in. Both paths are supported deliberately -
// the first is faster for a panel on the bench, the second is the only one that
// works for a panel already on a wall.
...
#define MENU_URL ""
#endif
#ifndef MENU_DEVICE_TOKEN
#define MENU_DEVICE_TOKEN ""
#endif
...
bool settingsLoad(Settings& out) {
  ...
  out.url = prefs.getString("url", MENU_URL);
  out.token = prefs.getString("token", MENU_DEVICE_TOKEN);
  prefs.end();
  return out.complete();
}
...
void failureCountSet(uint32_t n) {
  ...
  // every single wake, and NVS is flash - rewriting a zero seventeen times a
  // day for years is wear for no reason.
...
<h1>Panel de la minuta</h1><p>Configuraci&oacute;n de red</p>
...
<label>URL del servidor</label><input name=url value="%URL%" required>
<label>Token del dispositivo</label><input name=token value="%TOKEN%">
...
static void handleRoot() {
  ...
  page.replace("%URL%", escapeAttr(cur.url));
  page.replace("%TOKEN%", escapeAttr(cur.token));
...
static void handleSave() {
  ...
  if (server.hasArg("url")) s.url = server.arg("url");
  if (server.hasArg("token")) s.token = server.arg("token");
...
PortalInfo portalBegin() {
  ...
  char name[32];
  snprintf(name, sizeof(name), "AlfredMenu-%02X%02X", mac[4], mac[5]);
