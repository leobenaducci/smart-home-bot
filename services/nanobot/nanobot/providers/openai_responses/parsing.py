...
def _cached_from(details: Any) -> int:
    """`cached_tokens` out of an `input_tokens_details`, dict or SDK object.

    Both shapes reach here: the non-streaming path hands back parsed JSON and
    the streaming one hands back the SDK's model.
    """
    if not details:
        return 0
    value = (details.get("cached_tokens") if isinstance(details, dict)
             else getattr(details, "cached_tokens", None))
    # A number or nothing. `int()` on a stand-in object succeeds -- a MagicMock
    # converts to 1 -- so a provider or a test that hands back something which
    # is not a count would otherwise be recorded as one cached token.
...
def parse_response_output(response: Any) -> LLMResponse:
    ...
    if usage_raw:
        ...
        # The Responses API puts the cache count under `input_tokens_details`,
        # where `/chat/completions` uses `prompt_tokens_details`. This parser
        # read neither, so every model answering on /responses reported zero
        # cached tokens -- which looks exactly like a cache that is switched
        # off, and was read that way for a fortnight.
        cached = _cached_from(usage_raw.get("input_tokens_details"))
        if cached:
        ...
        if event_type == "response.output_item.added":
            ...
            if resp:
                ...
                if usage_obj:
                    ...
                    # Same field, streaming. Both paths had to be fixed: the
                    # non-streaming one is what a test exercises and the
                    # streaming one is what the house actually uses.
                    cached = _cached_from(
