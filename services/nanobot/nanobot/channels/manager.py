...
class ChannelManager:
    ...
    async def _send_once(channel: BaseChannel, msg: OutboundMessage) -> None:
        ...
        if (msg.metadata.get("_subagent_event") == "progress"
                and not channel.handles_subagent_progress):
            # Commentary for a channel that has no timeline to draw it in. It
            # would arrive as an empty message, hundreds of times per task, and
            # each failure would go through _send_with_retry's backoff. (start
            # and done are left alone: two per task, and pre-existing.)
            return
        ...
        while True:
            ...
            if same_target and is_delta and not final_metadata.get("_stream_end"):
                ...
                if is_end:
                    ...
                    # Carry the end message's payload across too. Adopting the
                    # flag while dropping _trim_to would leave the client showing
                    # the raw text it streamed, which is the bug send_delta was
                    # fixed for. Today's producers never set both flags on one
                    # message, so this is a guard, not a live path.
                    if "_trim_to" in (next_msg.metadata or {}):
                        final_metadata["_trim_to"] = next_msg.metadata["_trim_to"]
