...
class WebSocketChannel(BaseChannel):
    ...
    # `send()` turns these into subagent_status frames and relays them to
    # HomeCore (see the `_subagent_event` branch). It is the only channel that
    # does — see BaseChannel for what happens in the ones that do not.
    handles_subagent_progress = True
    ...
    def __init__(
        ...
        *,
        ...
        # Where output goes when no connection is subscribed to its chat_id.
        self._homeweb = HomeCoreRelay()
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        is_homeweb = self._homeweb.owns(msg.chat_id)

        # Sub-agent lifecycle events — emit as subagent_status, not a chat message.
        if msg.metadata.get("_subagent_event"):
            sub_event = msg.metadata["_subagent_event"]
            # `progress` carries the task's running commentary (see
            # _SubagentHook): what the model said to itself before each tool
            # call, and the tool calls with their outcome. It rides the same
            # envelope as start/done so channels learn one value, not a family.
            extra = {}
            if sub_event == "progress":
                extra = {
                    "kind": msg.metadata.get("kind", "thought"),
                    "text": msg.metadata.get("text", ""),
                    "detail": msg.metadata.get("detail", ""),
                    "seq": msg.metadata.get("seq", 0),
                }
            # Relayed whether or not anyone is listening: this is the record
            # HomeCore's background-task panel is built from, and a task that
            # starts with the app open still finishes with it closed. That
            # matters most for progress — these tasks run for hours, and a
            # trace that only exists while someone watches is one nobody sees.
            if is_homeweb:
                self._homeweb.post({
                    "type": "task",
                    "chat_id": msg.chat_id,
            ...
            for connection in conns:
                await self._safe_send_to(connection, raw, label=" subagent ")
            return

        # For a HomeCore chat, HomeCore is where this message *lives* — the day's
        # history, and the ntfy push when nobody is looking. The socket is how
        # it arrives quickly, not how it is kept.
        #
        # Relaying only when `conns` was empty made that backwards: durability
        # depended on whether a connection happened to be registered at this
        # instant, and a registered connection is not a reader. The reply
        # confirming two prize redemptions were marked delivered went out over a
        # socket whose app was already gone — never written to the history, so
        # the conversation sat on "I'll tell you when it's ready" while the work had
        # been done four minutes earlier.
        #
        # Posting on both paths is safe by construction: HomeCore dedupes against
        # what the page already stored (append_user_history) and only pushes
        # what it had not stored, so the cost of a live reader is one request,
        # not a duplicate message or a duplicate notification.
        #
        # Tool-call breadcrumbs and progress strings stay out: they are trace,
        ...
        if durable:
            ...
            # A fired reminder says which job it came from, so HomeCore can put
            # Listo / Descartar / Posponer on the notification.
            if msg.metadata.get("reminder_job"):
                relayed["reminder"] = {
                    "job": msg.metadata["reminder_job"],
                    "name": msg.metadata.get("reminder_name") or "",
                    "recurring": bool(msg.metadata.get("reminder_recurring")),
                }
            self._homeweb.post(relayed)
    ...
    async def send_delta(
        ...
        if meta.get("_stream_end"):
            ...
            # The runner strips skill-invocation blocks only after the text has
            # already gone out delta by delta, and reports here what should
            # remain. Dropping it left the client showing the raw
            # {"skill": ...} JSON it had painted live — and persisting it.
            trim_to = meta.get("_trim_to")
            if trim_to is not None:
                body["trim_to"] = trim_to
        else:
            body = {
                "event": "delta",
                "chat_id": chat_id,
