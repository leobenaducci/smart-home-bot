...
class MemoryStore:
    ...
    def append_history(
        ...
        *session_key* records which session the entry was consolidated from
        (``channel:chat_id``). ContextBuilder uses it to inject an entry back
        only into the session it came from — without it, every session's
        recent summaries leaked into every other session's system prompt.
        Dream ignores it and still processes everything.
...
class Consolidator:
    ...
    async def archive(
        ...
        try:
            ...
            # A job span, so this shows up in the profile as itself. It runs
            # inside somebody's turn and spends a whole prompt of its own;
            # unattributed it reads as that turn being mysteriously slow.
            with PROFILER.span("job", session_key or "", label="consolidate",
                               model=self.model or ""):
...
class Dream:
    ...
    def __init__(
        ...
        # See DreamConfig.write_skills. Read here once, at construction, because
        # the registry is built once.
        self.write_skills = write_skills
    ...
    def _build_tools(self) -> ToolRegistry:
        ...
        # Only when the household switched it on -- see DreamConfig.write_skills.
        if self.write_skills:
    ...
    async def run(self) -> bool:
        ...
        try:
            with PROFILER.span("job", label="dream:analyze", model=self.model or ""):
        ...
        messages: list[dict[str, Any]] = [
            {
                ...
                "content": render_template(
                    ...
                    write_skills=self.write_skills,
                ),
            },
