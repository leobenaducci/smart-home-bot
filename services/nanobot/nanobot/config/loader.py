...
def get_config_path() -> Path:
    ...
    if sys.platform == "win32":
        return Path(__file__).parents[2] / "config.json"
...
def _apply_profiling(config: Config) -> None:
    """Apply `profiling.enabled` to the module-singleton profiler.

    Here rather than in `serve()`, for the same reason the SSRF whitelist is
    here: there are four AgentLoop entrypoints (`nanobot.py`, `serve`, `run`,
    `chat`) and the profiler is reached by import from all of them. Set in one
    of them, `profiling.enabled: false` looked like it worked -- the endpoint
    that reports the flag lives in the process that honoured it -- while every
    other entrypoint went on recording session keys and chat ids. A switch
    that is off in the one place you can see it and on everywhere else is
    worse than no switch.
    """
    from nanobot.utils.profiling import PROFILER

    PROFILER.enabled = config.profiling.enabled
