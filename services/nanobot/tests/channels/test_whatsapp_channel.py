...
These are upstream's, about the mechanics either side of the decision to
answer: outbound media, sender-id resolution across the phone/LID split, the
LID→phone cache, voice transcription. They all pass `addressTrigger: ""`,
which is upstream's own behaviour of answering every allowed message — kept
reachable precisely so these stay tests of the mechanic and not of the policy
around it.

This house does not run that way. A real person's WhatsApp stores everything
and answers only a message that says Alfred's name, and that split has its own
file: tests/channels/test_whatsapp_store_and_answer.py.
"""
...
def _make_channel() -> WhatsAppChannel:
    bus = MagicMock()
    # replyPrefix "" keeps these about the payload shape. Marking a reply as
    # Alfred's is house policy and has its own tests in
    # test_whatsapp_store_and_answer.py; asserting it here would make every
    # media test depend on the wording of a prefix.
    ch = WhatsAppChannel({"enabled": True, "replyPrefix": ""}, bus)
    ch._ws = AsyncMock()
    ch._connected = True
    # These tests are about what goes over the wire once the decision to send
    # has been made. The decision itself is HomeCore's (`reply_mode` per chat)
    # and it fails closed, so without this every send test would be asserting
    # the gate rather than the payload. The gate has its own tests in
    # test_whatsapp_store_and_answer.py.
...
async def test_group_policy_mention_skips_unmentioned_group_message():
    ch = WhatsAppChannel(
        {"enabled": True, "groupPolicy": "mention", "addressTrigger": ""}, MagicMock())
...
async def test_sender_id_prefers_phone_jid_over_lid():
    ...
    ch = WhatsAppChannel({"enabled": True, "addressTrigger": ""}, MagicMock())
    ch._handle_message = AsyncMock()
