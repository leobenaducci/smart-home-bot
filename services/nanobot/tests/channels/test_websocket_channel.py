...
@pytest.mark.asyncio
async def test_stream_end_forwards_trim_to() -> None:
    """The runner strips skill-invocation blocks only after the text has already
    streamed, and reports the survivor as _trim_to. Dropping it here is what left
    raw {"skill": ...} JSON in the chat."""
    bus = MagicMock()
    channel = WebSocketChannel({"enabled": True, "allowFrom": ["*"], "streaming": True}, bus)
    mock_ws = AsyncMock()
    channel._attach(mock_ws, "chat-1")
    ...
    await channel.send_delta(
        "chat-1", "", {"_stream_end": True, "_stream_id": "s1", "_trim_to": "Listo."},
    )

    body = json.loads(mock_ws.send.call_args_list[0][0][0])
    assert body["event"] == "stream_end"
    assert body["trim_to"] == "Listo."


@pytest.mark.asyncio
async def test_stream_end_forwards_empty_trim_to() -> None:
    """An empty string is the meaningful case — the whole segment was the
    invocation block — so it must survive as "" and not collapse to absent,
    which the client reads as 'keep what you painted'."""
    bus = MagicMock()
    channel = WebSocketChannel({"enabled": True, "allowFrom": ["*"], "streaming": True}, bus)
    mock_ws = AsyncMock()
    channel._attach(mock_ws, "chat-1")
    ...
    await channel.send_delta(
        "chat-1", "", {"_stream_end": True, "_stream_id": "s1", "_trim_to": ""},
    )
...
@pytest.mark.asyncio
async def test_stream_end_omits_trim_to_when_absent() -> None:
    bus = MagicMock()
    channel = WebSocketChannel({"enabled": True, "allowFrom": ["*"], "streaming": True}, bus)
    mock_ws = AsyncMock()
    channel._attach(mock_ws, "chat-1")
