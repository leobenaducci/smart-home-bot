...
# --- what the rules stop at the outbound door -------------------------------
#
# email_rules decides; these are about the channel doing what it decided. The
# send path is where `respond` and `resend` stop being the same permission,
# because it is the only place the difference is decidable: back to whoever
# wrote is a reply, anywhere else is passing their mail to a third party.

from nanobot.channels.email_rules import evaluate as _evaluate  # noqa: E402


def _sent_by(channel, monkeypatch):
    """Capture what would have gone out, instead of opening SMTP."""
    sent = []
    monkeypatch.setattr(channel, "_smtp_send", lambda m: sent.append(m))
    return sent


def _armed(channel, sender, **rule):
    """Put a verdict in the channel for `sender`, as an inbound message would."""
    from email import policy
    from email.parser import Parser
    msg = Parser(policy=policy.default).parsestr("To: alfred@example.com\n\n")
    channel._verdict_by_chat[sender] = _evaluate(
        [{"match": "any", **rule}], sender=sender, headers=msg,
        alfred_addresses=["alfred@example.com"])


@pytest.mark.asyncio
async def test_a_reply_goes_out_when_respond_was_granted(monkeypatch):
    channel = EmailChannel(_make_config(rules_enforced=True), MessageBus())
    sent = _sent_by(channel, monkeypatch)
    _armed(channel, "jefe@x.cl", read=True, respond=True)
    channel._last_subject_by_chat["jefe@x.cl"] = "Consulta"

    await channel.send(OutboundMessage(channel="email", chat_id="jefe@x.cl", content="Listo."))
    assert len(sent) == 1 and sent[0]["To"] == "jefe@x.cl"


@pytest.mark.asyncio
async def test_a_reply_is_refused_when_only_read_was_granted(monkeypatch):
    channel = EmailChannel(_make_config(rules_enforced=True), MessageBus())
    sent = _sent_by(channel, monkeypatch)
    _armed(channel, "jefe@x.cl", read=True)
    channel._last_subject_by_chat["jefe@x.cl"] = "Consulta"

    await channel.send(OutboundMessage(channel="email", chat_id="jefe@x.cl", content="Listo."))
    assert sent == [], "read is not permission to answer"


@pytest.mark.asyncio
async def test_forwarding_needs_resend_and_not_respond(monkeypatch):
    """The distinction the two permissions exist for: answering the person who
    wrote is not the same act as sending their mail to somebody else."""
    channel = EmailChannel(_make_config(rules_enforced=True), MessageBus())
    sent = _sent_by(channel, monkeypatch)
    _armed(channel, "jefe@x.cl", read=True, respond=True)
    channel._last_subject_by_chat["jefe@x.cl"] = "Consulta"
    ...
    await channel.send(OutboundMessage(
        channel="email", chat_id="tercero@otra.cl", content="Mira esto",
        metadata={"in_reply_to_sender": "jefe@x.cl"}))
    assert sent == [], "respond does not authorise a forward"

    _armed(channel, "jefe@x.cl", read=True, respond=True, resend=True)
    await channel.send(OutboundMessage(
        channel="email", chat_id="tercero@otra.cl", content="Mira esto",
        metadata={"in_reply_to_sender": "jefe@x.cl"}))
    assert len(sent) == 1 and sent[0]["To"] == "tercero@otra.cl"


@pytest.mark.asyncio
async def test_an_address_no_rule_ever_covered_gets_nothing(monkeypatch):
    """Not an oversight to fall through: nothing granted anything here."""
    channel = EmailChannel(_make_config(rules_enforced=True), MessageBus())
    sent = _sent_by(channel, monkeypatch)

    await channel.send(OutboundMessage(channel="email", chat_id="desconocido@x.cl", content="Hola"))
    assert sent == []


@pytest.mark.asyncio
async def test_force_send_does_not_route_around_a_withheld_permission(monkeypatch):
    """`force_send` exists so a proactive message is not mistaken for an
    auto-reply. A permission somebody withheld is not an inconvenience."""
    channel = EmailChannel(_make_config(rules_enforced=True), MessageBus())
    sent = _sent_by(channel, monkeypatch)
    _armed(channel, "jefe@x.cl", read=True)

    await channel.send(OutboundMessage(channel="email", chat_id="jefe@x.cl", content="Igual te escribo",
                                       metadata={"force_send": True}))
    assert sent == []


@pytest.mark.asyncio
async def test_with_enforcement_off_nothing_changes(monkeypatch):
    """Every deployment that has this channel today has no rules. Turning the
    file on must not turn their email off."""
    channel = EmailChannel(_make_config(rules_enforced=False), MessageBus())
    sent = _sent_by(channel, monkeypatch)
    channel._last_subject_by_chat["quien@sea.cl"] = "Hola"

    await channel.send(OutboundMessage(channel="email", chat_id="quien@sea.cl", content="Respuesta"))
    assert len(sent) == 1


@pytest.mark.asyncio
async def test_on_request_refuses_an_auto_reply_and_allows_a_requested_one(monkeypatch):
    """«Contesta sólo lo que la familia le pida.» An email arriving is not a
    request — and an auto-reply is by definition the unrequested kind, which is
    exactly what this mode exists to stop."""
    channel = EmailChannel(_make_config(rules_enforced=True), MessageBus())
    sent = _sent_by(channel, monkeypatch)
    _armed(channel, "jefe@x.cl", read=True, respond_mode="on_request")
    channel._last_subject_by_chat["jefe@x.cl"] = "Consulta"

    await channel.send(OutboundMessage(channel="email", chat_id="jefe@x.cl",
                                       content="respuesta automática"))
    assert sent == [], "nobody asked for this one"

    await channel.send(OutboundMessage(channel="email", chat_id="jefe@x.cl",
                                       content="respuesta pedida",
                                       metadata={"asked_by": "user1"}))
    assert len(sent) == 1


@pytest.mark.asyncio
async def test_nothing_goes_out_until_the_text_is_approved(monkeypatch):
    """«Confirma en la app antes de compartir datos personales.» The gate is
    mechanical rather than a prompt convention: the model can be told to ask,
    and a model that forgets would otherwise send."""
    channel = EmailChannel(_make_config(rules_enforced=True), MessageBus())
    sent = _sent_by(channel, monkeypatch)
    _armed(channel, "jefe@x.cl", read=True, respond_mode="always", confirm=True)
    channel._last_subject_by_chat["jefe@x.cl"] = "Consulta"

    await channel.send(OutboundMessage(channel="email", chat_id="jefe@x.cl",
                                       content="el RUT es 12.345.678-9"))
    assert sent == [], "no approval, no send"

    await channel.send(OutboundMessage(channel="email", chat_id="jefe@x.cl",
                                       content="el RUT es 12.345.678-9",
                                       metadata={"approved_draft": "msg-441"}))
    assert len(sent) == 1


@pytest.mark.asyncio
async def test_approval_is_required_even_for_a_reply_somebody_asked_for(monkeypatch):
    """The two conditions are independent: asking for a reply is not approving
    its wording, which is the whole point when the wording is the risk."""
    channel = EmailChannel(_make_config(rules_enforced=True), MessageBus())
    sent = _sent_by(channel, monkeypatch)
    _armed(channel, "jefe@x.cl", read=True, respond_mode="on_request", confirm=True)
    channel._last_subject_by_chat["jefe@x.cl"] = "Consulta"

    await channel.send(OutboundMessage(channel="email", chat_id="jefe@x.cl",
                                       content="ahí va", metadata={"asked_by": "user1"}))
    assert sent == []

    await channel.send(OutboundMessage(
        channel="email", chat_id="jefe@x.cl", content="ahí va",
        metadata={"asked_by": "user1", "approved_draft": "msg-9"}))
    assert len(sent) == 1
