...
class TestDreamRun:
    ...
    async def test_skill_phase_uses_builtin_skill_creator_path(self, mock_provider, mock_runner, store):
        """With skill writing on, Dream points skill creation at the builtin skill-creator template."""
    ...
    async def test_dream_cannot_write_skills_by_default(self, dream, store, mock_provider, mock_runner):
        """A nightly job must not author standing instructions unreviewed.

        On 2026-06-06 Dream wrote `direct-curl-execution` -- "bypass skills
        with curl via exec when they fail" -- into a member's live workspace,
        and every prompt after that taught the detours it described.
        """
        assert dream._tools.get("write_file") is None
        store.append_history("Repeated workflow one")
        mock_provider.chat_with_retry.return_value = MagicMock(content="[SKILL] test-skill: x")
        mock_runner.run = AsyncMock(return_value=_make_run_result())
        await dream.run()
        prompt = mock_runner.run.call_args[0][0].initial_messages[0]["content"]
        assert "do NOT create skills" in prompt
        assert "Use write_file to create skills" not in prompt

    async def test_skill_write_tool_accepts_workspace_relative_skill_path(self, store, mock_provider, mock_runner):
        """With write_skills on, skills/<name>/SKILL.md relative to the workspace root is allowed."""
        dream = Dream(store=store, provider=mock_provider, model="test-model", max_batch_size=5,
                      write_skills=True)
        dream._runner = mock_runner
