...
def test_subagent_followup_carries_reasoning_content() -> None:
    """The injected result must not be the message that kills the next turn.

    A subagent's answer is written into history as an *assistant* message by
    this process, so nothing thought about it and nothing set
    `reasoning_content`. A thinking model then refuses the following request —
    "The `reasoning_content` in the thinking mode must be passed back to the
    API" (400) — and since this message is appended right before the loop asks
    the model to react to it, the failure lands on the very turn that was
    supposed to relay the result.

    Observed on 2026-08-13 in Alex's finanzas chat: the subagent finished, the
    result was injected, and the turn died with the raw provider error.
    """
    loop = _mk_loop()
    session = Session(key="cli:reasoning")
    msg = InboundMessage(
        channel="system",
        sender_id="subagent",
        chat_id="cli:reasoning",
        content="subagent result",
        metadata={"subagent_task_id": "sub-1"},
    )

    assert loop._persist_subagent_followup(session, msg) is True
    assert session.messages[-1]["reasoning_content"] == ""


def test_interrupted_turn_placeholder_carries_reasoning_content() -> None:
    """Crash recovery must not leave a message the next request chokes on."""
    loop = _mk_loop()
    session = Session(key="cli:interrupted")
    session.metadata[AgentLoop._PENDING_USER_TURN_KEY] = True
    session.add_message("user", "¿cómo va?")
