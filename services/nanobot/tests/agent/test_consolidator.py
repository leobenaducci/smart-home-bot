...
class TestConsolidatorSummarize:
    ...
    async def test_archive_tags_entry_with_session_key(self, consolidator, mock_provider, store):
        """The history entry must record which session it was consolidated from."""
        mock_provider.chat_with_retry.return_value = MagicMock(
        ...
        entries = store.read_unprocessed_history(since_cursor=0)
        assert entries[0]["session"] == "websocket:homeweb:user1:2026-07-31:1000"

    async def test_raw_fallback_tags_entry_with_session_key(self, consolidator, mock_provider, store):
        """The raw-dump fallback must carry the session tag too."""
        mock_provider.chat_with_retry.side_effect = Exception("API error")
        await consolidator.archive(
            [{"role": "user", "content": "hi"}], session_key="telegram:123",
        )
        entries = store.read_unprocessed_history(since_cursor=0)
        assert entries[0]["session"] == "telegram:123"

    async def test_archive_without_session_key_leaves_entry_untagged(self, consolidator, mock_provider, store):
        """No session_key (e.g. /new snapshots) → no tag, so the entry reaches
        Dream but is never re-injected into any keyed session's prompt."""
        mock_provider.chat_with_retry.return_value = MagicMock(
            content="Summary.", finish_reason="stop",
        )
