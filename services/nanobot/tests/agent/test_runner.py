...
async def test_loop_stream_filter_handles_think_only_prefix_without_crashing(tmp_path):
    ...
    async def on_stream_end(*, resuming: bool = False, trim_to: str | None = None) -> None:
        endings.append(resuming)
        trims.append(trim_to)
    ...
    assert deltas == ["Hello"]
    assert endings == [False]
    # The correction the client repaints with must be the visible answer, not
    # the raw text it streamed — otherwise the reasoning block comes back.
    assert trims == ["Hello"]
...
def test_snip_history_block_limit_budgets_history_not_the_system_prompt(monkeypatch):
    """Regression (2026-09-22): the limit is history's budget. A system prompt
    bigger than it used to leave 128 tokens for the conversation, so every turn
    reached the model as its newest message alone."""
    from nanobot.agent.runner import AgentRunSpec, AgentRunner
    ...
    spec = AgentRunSpec(
        ...
        context_block_limit=16_000,
    )
    monkeypatch.setattr(
        "nanobot.agent.runner.estimate_prompt_tokens_chain", lambda *_a, **_kw: (32_000, None))
    monkeypatch.setattr(
        "nanobot.agent.runner.estimate_message_tokens",
        lambda msg: 31_000 if msg.get("role") == "system" else 300,
    )

    assert runner._snip_history(spec, messages) == messages

    # Over the limit it still trims, against the history alone.
    spec.context_block_limit = 700
    trimmed = runner._snip_history(spec, messages)
    assert [m["content"] for m in trimmed] == ["system", "check again"]
...
@pytest.mark.asyncio
async def test_text_parsed_json_skill_triggers_read_file(tmp_path):
    """JSON skill invocation in model text is converted to a read_file call."""
    from nanobot.agent.runner import AgentRunSpec, AgentRunner
    ...
    tools.execute = AsyncMock(return_value="# Test Skill content")

    runner = AgentRunner(provider)
    result = await runner.run(AgentRunSpec(
        initial_messages=[
            {"role": "system", "content": system_msg},
            {"role": "user", "content": "use test skill"},
        ],
        tools=tools,
        model="test-model",
        max_iterations=5,
    ...
    assert result.final_content == "done"
    assert "read_file" in result.tools_used
    tools.execute.assert_any_call("read_file", {"path": str(skill_md)})


@pytest.mark.asyncio
async def test_text_parsed_json_skill_with_action_no_python_guide_falls_back_to_read_file(tmp_path):
    """JSON skill invocation with action args but no SKILL_PYTHON.md falls back to read_file."""
    from nanobot.agent.runner import AgentRunSpec, AgentRunner

    skill_dir = tmp_path / "my-skill"
    skill_dir.mkdir()
    skill_md = skill_dir / "SKILL.md"
    skill_md.write_text("# My Skill\nNo python guide here.\n")
    ...
    async def chat_with_retry(*, messages, **kwargs):
        ...
        if call_count["n"] == 1:
            return LLMResponse(
                content='{"skill": "my-skill", "action": "run", "param": "value"}',
                tool_calls=[],
                usage={},
            )
        return LLMResponse(content="done", tool_calls=[], usage={})
    ...
    tools.get_definitions.return_value = []
    tools.tool_names = ["read_file"]
    tools.execute = AsyncMock(return_value="# My Skill content")

    runner = AgentRunner(provider)
    result = await runner.run(AgentRunSpec(
        initial_messages=[
            {"role": "system", "content": system_msg},
            {"role": "user", "content": "use my skill"},
        ],
        tools=tools,
        model="test-model",
        max_iterations=5,
...
# Skill path routing
#
# The name→SKILL.md map is parsed out of the system message; without it a
# {"skill": ...} block cannot be resolved and gets published to the chat as raw
# JSON instead of being executed. A backtick in a description used to break the
# parse, which silently unrouted camera-feed, geo and github.
# ---------------------------------------------------------------------------
def test_skill_path_parsed_when_description_contains_backticks():
    from nanobot.agent.runner import _collect_skill_paths

    summary = "\n".join([
        "- **geo** — usa `where_is` y devuelve `mapa`  `nanobot/skills/geo/SKILL.md`",
        "- **camera-feed** — `camera` es el nombre  `nanobot/skills/camera-feed/SKILL.md`",
        "- **tasks** — tareas de la casa  `nanobot/skills/tasks/SKILL.md`",
        "- **n8n** — flujos (unavailable: N8N_URL)  `nanobot/skills/n8n/SKILL.md`",
    ])
    paths = _collect_skill_paths([{"role": "system", "content": summary}])

    assert paths["geo"] == "nanobot/skills/geo/SKILL.md"
    assert paths["camera-feed"] == "nanobot/skills/camera-feed/SKILL.md"
    assert paths["tasks"] == "nanobot/skills/tasks/SKILL.md"
    assert paths["n8n"] == "nanobot/skills/n8n/SKILL.md"


def test_skill_path_prefers_the_emitted_path_over_a_mention():
    from nanobot.agent.runner import _collect_skill_paths

    summary = "- **my** — lee el `SKILL.md` primero  `nanobot/skills/my/SKILL.md`"
    paths = _collect_skill_paths([{"role": "system", "content": summary}])

    assert paths["my"] == "nanobot/skills/my/SKILL.md"


def test_prose_is_not_mistaken_for_a_skill_path_line():
    from nanobot.agent.runner import _collect_skill_paths

    paths = _collect_skill_paths(
        [{"role": "system", "content": "Read the `SKILL.md` for details."}]
    )

    assert paths == {}


def test_skill_invocation_block_stripped_when_response_has_tool_calls():
    """Content ships to the user alongside a structured call, so an invocation
    block written beside one must not reach the chat as raw JSON."""
    from nanobot.agent.runner import _without_skill_invocation_text

    response = LLMResponse(
        content='Acá tienes:\n\n```json\n{"skill": "geo", "action": "locate"}\n```',
        tool_calls=[ToolCallRequest(id="1", name="exec", arguments={"command": "ls"})],
        finish_reason="tool_calls",
    )
    out = _without_skill_invocation_text(response)

    assert "skill" not in out.content
    assert "Acá tienes:" in out.content
    assert out.tool_calls == response.tool_calls


# --- a skill invocation that resolves to nothing -----------------------------
#
# The runner already detected this case and only wrote a log line. Its own
# comment predicted the consequence exactly — «the person is left with the
# sentence and nothing to report but "no hizo nada"» — and that is how it
# reached us: a report about a checkout that never ran, from a turn that ended
# quietly. A skill can be absent for an ordinary reason, because the ones
# served by their own service materialise only while that service answers, so
# `code` is missing whenever the code-broker is down. That is a fact about the
# house and it belongs in the answer.

from nanobot.agent.runner import (  # noqa: E402
    _available_skill_names,
    _named_skill_invocations,
)


def test_the_skill_name_is_recovered_from_the_block():
    content = 'Voy a mirarlo.\n{"skill": "code", "action": "checkout", "slug": "fracciones"}'
    assert _named_skill_invocations(content) == ["code"]


def test_a_fenced_block_counts_too():
    content = 'Listo.\n```json\n{"skill": "family", "action": "list"}\n```'
    assert _named_skill_invocations(content) == ["family"]


def test_ordinary_prose_names_nothing():
    assert _named_skill_invocations("Te dejo el resumen y nada más.") == []
    assert _named_skill_invocations(None) == []


def test_available_skills_are_read_off_the_disk(tmp_path):
    """Read from the filesystem rather than asked of the loader: on this path
    what matters is what is there *now*, and a remote skill comes and goes with
    the service that serves it."""
    ws = tmp_path / "workspace"
    (ws / "skills" / "propia").mkdir(parents=True)
    (ws / "skills" / "propia" / "SKILL.md").write_text("# propia\n")
    (ws / ".skills-remote" / "code").mkdir(parents=True)
    (ws / ".skills-remote" / "code" / "SKILL.md").write_text("# code\n")
    # A directory with no SKILL.md is not a skill.
    (ws / "skills" / "vacia").mkdir()

    class _Spec:
        workspace = ws

    names = _available_skill_names(_Spec())
    assert "propia" in names and "code" in names
    assert "vacia" not in names


def test_a_missing_workspace_is_no_skills_not_an_error(tmp_path):
    class _Spec:
        workspace = tmp_path / "does-not-exist"

    # The builtins still resolve; the point is that it does not raise.
    assert isinstance(_available_skill_names(_Spec()), set)
