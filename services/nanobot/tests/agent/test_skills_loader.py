...
def test_a_skill_that_cannot_run_is_named_not_described(tmp_path: Path) -> None:
    """Its full description is prompt spent on every turn for something the model
    cannot invoke; one line still lets it say "not on this machine"."""
    workspace = tmp_path / "ws"
    ws_skills = workspace / "skills"
    ws_skills.mkdir(parents=True)
    _write_skill(ws_skills, "alpha", body="# Alpha")
    _write_skill(ws_skills, "ghost", metadata_json={"requires": {"bins": ["no-such-binary-xyz"]}},
                 body="# Ghost")
    builtin = tmp_path / "builtin"
    builtin.mkdir()
    ...
    assert summary.splitlines()[-1].startswith("- Not available on this machine: ghost (")
