...
class TestMessageToolSuppressLogic:
    ...
    async def test_suppress_when_sent_to_same_target(self, tmp_path: Path) -> None:
        ...
        assert len(sent) == 1
        # Not None: process_direct callers never read the bus, so they get the
        # MessageTool content back. `_already_sent` is what stops the run loop
        # from publishing it a second time — see the duplicate-delivery test.
        assert result is not None
        assert result.content == "Hello"
        assert result.metadata.get("_already_sent") is True
