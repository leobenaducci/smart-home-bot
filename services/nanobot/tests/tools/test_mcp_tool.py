...
@pytest.mark.asyncio
@pytest.mark.parametrize("entry", ["demo", "mcp_test_demo"])
async def test_connect_mcp_servers_disabled_tools_are_never_registered(
    fake_mcp_runtime: dict[str, object | None], entry: str,
) -> None:
    fake_mcp_runtime["session"] = _make_fake_session(["demo", "other"])
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", disabled_tools=[entry])},
        registry,
    )
    for stack in stacks.values():
        await stack.aclose()
...
@pytest.mark.asyncio
async def test_connect_mcp_servers_disabled_tools_win_over_enabled_tools(
    fake_mcp_runtime: dict[str, object | None],
) -> None:
    fake_mcp_runtime["session"] = _make_fake_session(["demo", "other"])
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", enabled_tools=["demo", "other"],
                                 disabled_tools=["demo"])},
        registry,
    )
    for stack in stacks.values():
...
@pytest.mark.asyncio
async def test_connect_mcp_servers_disabled_tools_warns_on_unknown_entries(
    fake_mcp_runtime: dict[str, object | None], monkeypatch: pytest.MonkeyPatch
) -> None:
    """A misspelt denylist entry leaves the tool on, so it has to be said."""
    fake_mcp_runtime["session"] = _make_fake_session(["demo"])
    registry = ToolRegistry()
    warnings: list[str] = []
    ...
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", disabled_tools=["demoo"])},
        registry,
    )
    for stack in stacks.values():
        await stack.aclose()
    ...
    assert registry.tool_names == ["mcp_test_demo"]
    assert any("disabledTools entries not found: demoo" in w for w in warnings)
...
@pytest.mark.asyncio
async def test_disabled_tools_still_match_after_home_assistant_prefixes_them(
    fake_mcp_runtime: dict[str, object | None], monkeypatch: pytest.MonkeyPatch
) -> None:
    """Home Assistant's MCP server began serving `todo__HassListAddItem` and
    `todo__get_items` where the config says `HassListAddItem` and
    `todo_get_items`. Exact matching let the to-do tools back in, and the
    chores question was answered from HA's empty list."""
    fake_mcp_runtime["session"] = _make_fake_session(
        ["todo__get_items", "todo__HassListAddItem", "intent__HassTurnOn"])
    registry = ToolRegistry()
    warnings: list[str] = []
    monkeypatch.setattr("nanobot.agent.tools.mcp.logger.warning",
                        lambda message, *args: warnings.append(message.format(*args)))

    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake",
                                 disabled_tools=["todo_get_items", "HassListAddItem"])},
        registry,
    )
    for stack in stacks.values():
        await stack.aclose()

    assert registry.tool_names == ["mcp_test_intent__HassTurnOn"]
    assert not any("disabledTools entries not found" in w for w in warnings), warnings


# --- resources and prompts obey the same two lists ---------------------------
# They are offered to the model as tools, and were registered after the filter
# whatever it said: Bright Data's prompt helpers and Home Assistant's `Assist`
# prompt and context snapshot rode into every turn unasked.

def _make_fake_session_with_extras(tools, prompts, resources) -> SimpleNamespace:
    session = _make_fake_session(tools)

    async def list_prompts() -> SimpleNamespace:
        return SimpleNamespace(prompts=[SimpleNamespace(name=p, description=p, arguments=[])
                                        for p in prompts])

    async def list_resources() -> SimpleNamespace:
        return SimpleNamespace(resources=[SimpleNamespace(name=r, description=r, uri=f"x://{r}",
                                                          mimeType="text/plain")
                                          for r in resources])
...
@pytest.mark.asyncio
async def test_an_enabled_tools_list_leaves_out_prompts_and_resources_it_does_not_name(
    fake_mcp_runtime: dict[str, object | None],
) -> None:
    fake_mcp_runtime["session"] = _make_fake_session_with_extras(
        ["search_engine"], ["web_scraping_strategy"], ["snapshot"])
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"bd": MCPServerConfig(command="fake", enabled_tools=["search_engine"])}, registry)
    for stack in stacks.values():
        await stack.aclose()
    assert registry.tool_names == ["mcp_bd_search_engine"]


@pytest.mark.asyncio
async def test_disabled_tools_take_out_a_prompt_and_a_resource_by_wrapped_name(
    fake_mcp_runtime: dict[str, object | None],
) -> None:
    fake_mcp_runtime["session"] = _make_fake_session_with_extras(
        ["HassTurnOn"], ["Assist", "Other"], ["assist_context_snapshot"])
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"ha": MCPServerConfig(command="fake", disabled_tools=[
            "mcp_ha_prompt_Assist", "mcp_ha_resource_assist_context_snapshot"])}, registry)
    for stack in stacks.values():
        await stack.aclose()
    assert sorted(registry.tool_names) == ["mcp_ha_HassTurnOn", "mcp_ha_prompt_Other"]
