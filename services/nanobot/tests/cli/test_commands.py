...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    # Where it went and what it said. Deliberately not an equality on the whole
    # OutboundMessage: this test is about the evaluator getting the scheduled
    # task's context, and `metadata` belongs to the notification's buttons —
    # `3756a85` added reminder_job/name/recurring so a fired reminder can be
    # answered instead of only read (websocket.py reads them back), and this
    # assertion failed on a feature it has no opinion about.
    bus.publish_outbound.assert_awaited_once()
    sent = bus.publish_outbound.await_args.args[0]
    assert sent.channel == "telegram"
    assert sent.chat_id == "user-1"
    assert sent.content == "Time to stretch."
