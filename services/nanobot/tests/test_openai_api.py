...
async def test_successful_request_uses_fixed_api_session(aiohttp_client, mock_agent) -> None:
    ...
    mock_agent.process_direct.assert_called_once_with(
        ...
        channel="api",
        chat_id=API_CHAT_ID,
        # A request that does not ask for the stronger model does not get it,
        # and one that names no Profesión gets the default roster entry.
        powerful=False,
        profile=None,
    )
...
async def test_process_direct_accepts_media() -> None:
    ...
    async def fake_process(msg, *, session_key="", on_progress=None, on_stream=None,
                           on_stream_end=None, powerful=False, profile=None):
