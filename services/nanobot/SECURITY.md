<!-- home-stack: local banner. Everything below is upstream text; leave it as
     upstream when pulling. -->
> **Upstream document.** This describes stock
> [HKUDS/nanobot](https://github.com/HKUDS/nanobot), not this deployment. This
> tree is a fork with substantial local changes, and most of what follows about
> installing, configuring or running it does not apply here.
> **Read [`AGENTS.md`](AGENTS.md) instead** for what this stack does with
> nanobot.
>
> **A vulnerability in *your* deployment is not an upstream report.** The
> address below reaches the upstream maintainers, who did not write the
> local changes and cannot fix them. Handle it in this repository first.
...
- Shared-secret authentication between Python and Node.js is always on: both sides read or create `~/.nanobot/whatsapp-auth/bridge-token` (mode 0600) when no token is configured. Setting `bridgeToken` in config, or `BRIDGE_TOKEN` in the bridge's environment, overrides it — but only if both sides are set to the same value
- Keep authentication data in `~/.nanobot/whatsapp-auth` secure (mode 0700) — it holds both the linked-device session and that shared secret
...
- WhatsApp bridge: localhost-only binding + mandatory token auth
