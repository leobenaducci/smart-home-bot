...
// Prefer Opus/WebM (Chrome), fall back to Ogg or MP4.
...
const WHISPER_URL = "http://whisper.home:8000/transcribe";
...
export function ThreadComposer({
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        fetch(WHISPER_URL, { method: "POST", body: form })
          ...
          .catch(() => {/* transcription failed — silently discard */})
  ...
  return (
    ...
    >
      ...
      >
        {(images.length > 0 || recordingState !== "idle") ? (
          <div
          ...
          >
            ...
            {recordingState !== "idle" && (
              <AudioChip
                ...
                labelCancel={t("thread.composer.removeAudio")}
                onCancel={recordingState === "recording" ? cancelRecording : undefined}
              />
            )}
          </div>
        ) : null}
          ...
          <div className="flex min-w-0 items-center gap-2">
            ...
            <Button
              ...
              aria-label={
                recordingState === "recording"
                  ? t("thread.composer.stopRecording")
                  : t("thread.composer.recordAudio")
              }
            ...
            >
              {recordingState === "transcribing"
                ...
                : recordingState === "recording"
                  ? <Square className={cn("fill-current", isHero ? "h-3.5 w-3.5" : "h-3 w-3")} />
...
function AudioChip({ state, labelCancel, onCancel }: AudioChipProps) {
  ...
  return (
    ...
    >
      <div className="flex h-10 w-10 items-center justify-center rounded-md bg-background">
        {state === "recording"
          ? <Mic className="h-4 w-4 animate-pulse text-red-500" aria-hidden />
      ...
      <div className="flex min-w-0 flex-col text-[11.5px] leading-4">
        ...
        {state === "recording" && (
          <span className="text-muted-foreground">{formatDuration(liveSecs)}</span>
        )}
      </div>
      {onCancel && (
        <button
